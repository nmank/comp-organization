...
int divpwr2(int x, int n) {
    //shift it over n (does not work for odd, negative ints)
    int xnext = x >> n;
    //deal with the negative non-divisible
    int neg = 1<<31;
    int non_dv = ~((~0) << n);
    int chk_neg = neg & x;
    int chk_div = non_dv & x;
    int offset = !(!chk_neg + !chk_div);
    //add the offset if n is non-zero
    int result = xnext + offset;
